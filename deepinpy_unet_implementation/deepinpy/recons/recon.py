        ...
        if _idx is not None:
            with torch.no_grad():
                ...
                for index in range(3):
                  _x_hat = utils.t2n(x_hat[index,...])
                  _x_gt = utils.t2n(imgs[index,...])
                  _x_adj = utils.t2n(x_adj[index,...])
                  ...
                  if self.logger:
                    self.logger.experiment.add_image(str(index) + '_train_prediction', grid, self.current_epoch)

                  if self.current_epoch == 0:
                      myim = torch.tensor(np.stack((np.abs(_x_gt), np.angle(_x_gt)), axis=0))[:, None, ...]
                      grid = make_grid(myim, scale_each=True, normalize=True, nrow=8, pad_value=10)
                      if self.logger:
                          self.logger.experiment.add_image(str(index) + '_ground_truth', grid, 0)

                      myim = torch.tensor(np.stack((np.abs(_x_adj), np.angle(_x_adj)), axis=0))[:, None, ...]
                      grid = make_grid(myim, scale_each=True, normalize=True, nrow=8, pad_value=10)
                      if self.logger:
                          self.logger.experiment.add_image(str(index) + '_input', grid, 0)
